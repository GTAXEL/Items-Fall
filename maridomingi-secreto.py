...
def resource_path(relative_path):
    try:
        base_path = sys._MEIPASS  # Ruta temporal generada por PyInstaller
    except Exception:
...
# Cargar sonidos con rutas compatibles con PyInstaller
...
def draw_boss_health_bar():
    if boss_alive:
        ...
        pygame.draw.rect(win, GREEN, (boss_x - 10, boss_y - 30, health_bar_width * (boss_health / 100), health_bar_height))
...
while running:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            running = False
        if event.type == ENEMY_EVENT and lives > 0:  # Evitar que los enemigos caigan cuando ya no hay vidas
            enemy_x = random.randint(0, WIDTH - player_size)
            enemies.append(pygame.Rect(enemy_x, 0, player_size, player_size))
        if event.type == POWERUP_EVENT and lives > 0:  # Evitar que los power-ups caigan cuando ya no hay vidas
    ...
    if lives > 0:  # Solo permitir movimiento y disparos si el jugador tiene vidas
    ...
    for enemy in enemies[:]:
        ...
        if player_rect.colliderect(enemy) and lives > 0:  # Evitar perder vidas si ya no quedan
            lives -= 1
    ...
    if score >= 100 and not boss_alive:
        boss_alive = True
    ...
    if lives <= 0:
        ...
        # Esperar hasta que el jugador presione ESC o cierre la ventana
        keys = pygame.key.get_pressed()
